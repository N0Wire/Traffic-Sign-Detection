...
def train(model, dataloader, n_epochs=10, checkpoint_name='training', use_gpu=True, stn=True, dataloader_test=None, logger=None):
    ...
    for epoch in tqdm(range(n_epochs), desc='epoch', position=1):
        ...
        if epoch > 0:
            visualize_scalar(loss_list, filename="loss.pdf", title="Loss of total network", xname="batch", 
                         yname="loss", show=False, scalars=1, labels=None, ylim=(0,6))
        
            visualize_scalar(batch_acc_list, filename="batch_acc.pdf", title="Accuracy of batch", xname="batch", 
                         yname="accuracy", show=False, scalars=1, labels=None)
        
            if dataloader_test is None:
                visualize_scalar(model.list_train_acc, filename="train_acc.pdf", title="Accuracy of Trainset", xname="epoch", 
                             yname="accuracy", show=False, scalars=1, labels=None)
            else:
                ...
                visualize_scalar(data, filename="train_test_acc.pdf", title="Accuracy of Datasets", xname="epoch", 
